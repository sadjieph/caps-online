...
Batch berguna kalau kamu menjalankan pelatihan beberapa kali (mis. "Batch Agustus 2026", "Leadership Kelas B") dan ingin hasilnya terpisah per kelompok.

**Cara pakai (dari Dashboard Admin):**
1. Buka Admin → bagian **"Kelola Batch Peserta"** → ketik nama batch → klik **+ Tambah Batch**.
2. Setiap batch yang dibuat akan muncul sebagai baris dengan tombol **Salin Link**.
3. Klik **Salin Link**, lalu bagikan link tersebut ke peserta batch itu (mis. lewat WhatsApp Group).
4. Peserta yang membuka link tersebut otomatis tercatat ke batch itu — mereka **tidak perlu memilih apa pun**, cukup isi nama dan jawab kuesioner seperti biasa.
5. Kalau peserta membuka link biasa (tanpa `?batch=...`) sementara sudah ada batch yang dibuat, mereka akan diminta memilih batch dari dropdown sebelum mengisi.
6. Di tabel dan pengelompokan hasil pada Dashboard Admin, gunakan dropdown **"Semua Batch"** di toolbar untuk menyaring tampilan (statistik, 4 kolom hasil, tabel, dan tombol Reset) hanya untuk satu batch tertentu.
7. Menghapus batch (ikon 🗑 di baris batch) hanya menghapus batch itu sendiri — jawaban peserta yang sudah masuk tetap tersimpan, hanya link-nya saja yang tidak berlaku lagi.
...
```
Setiap dokumen di koleksi `batches` berisi:
```json
{
